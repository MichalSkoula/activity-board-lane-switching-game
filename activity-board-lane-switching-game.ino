...
#define JOYSTICK_THRESHOLD 400 // Increased threshold to make joystick less sensitive
...
bool gameOver = false;
int currentDifficulty = 1; // Current difficulty level (1-5)
int obstacleSpeed = 2;     // Starting obstacle speed
int spawnInterval = 4500;  // Starting spawn interval in ms
int levelUpEvery = 5;      // Level up every 5 points
...
void setup()
{
  ...
  // Rotate display 180 degrees (flip it upside down)
...
void loop()
{
  ...
  { // Increased from 50ms to 300ms to reduce sensitivity
  ...
  // Spawn obstacles with dynamic interval based on difficulty
  if (millis() - lastObstacleSpawn > spawnInterval)
  {
    spawnObstacle();
  ...
  drawObstacles();
  drawScore();
  drawDifficulty(); // Add difficulty indicator
  display.display();

  // Dynamic game delay based on score
  int gameDelay = max(20, GAME_SPEED - (score / 15)); // Faster difficulty increase
  delay(gameDelay);
}
...
void updateObstacles()
{
  for (int i = 0; i < MAX_OBSTACLES; i++)
  {
    if (obstacles[i].active)
    {
      // Move obstacle down based on current dynamic speed
...
void drawDifficulty()
{
  ...
  display.setTextSize(1);
  display.print("L");
  display.print(currentDifficulty);
}
...
// Sound for difficulty increase
void playDifficultyUpSound()
{
  // Three ascending tones
  for (int i = 1000; i <= 2000; i += 500)
  {
    tone(BUZZER_PIN, i, 80);
    delay(100);
  }
}

// New function to update difficulty based on score
void updateDifficulty()
{
  // Calculate new difficulty level (1-5)
  int newDifficulty = 1 + min(4, score / levelUpEvery); // Level up every 5 points, max level 5

  // If difficulty has changed
  if (newDifficulty != currentDifficulty)
  {
    currentDifficulty = newDifficulty;

    // Update game parameters based on difficulty - more dramatic increases
    obstacleSpeed = 2 + (currentDifficulty - 1);
    spawnInterval = 4500 - ((currentDifficulty - 1) * 800); // Much larger spawn interval reduction

    // Play sound effect for difficulty increase
...
void resetGame()
{
  ...
  currentDifficulty = 1; // Reset difficulty
  obstacleSpeed = 2;     // Reset obstacle speed
  spawnInterval = 4500;  // Reset spawn interval
